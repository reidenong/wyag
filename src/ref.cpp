...
std::optional<std::string> resolve_ref(const GitRepository& repo, fs::path path) {
    ...
    if (length >= 5 && std::string_view {start, 5} == "ref: ") {
        return resolve_ref(repo, repo.get_gitdir() / fs::path {std::string_view {start + 5, length - 5}});
    }
